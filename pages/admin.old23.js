...
export default function AdminPage() {
  ...
  function extractStoragePathFromUrl(url) {
    try { const m = String(url).match(/\/o\/([^?]+)/); return m ? decodeURIComponent(m[1]) : null; } catch { return null; }
  }
        ...
        <>
          <div className="card" style={{ marginBottom: 12 }}>
            ...
            <div className="row no-print" style={{ gap: 8, marginTop: 8 }}>
              ...
              <button className="btn outline" onClick={() => handleDelete(selected.uid, selected.clientName || selected.email)} disabled={deletingId === selected.uid}>
                {deletingId === selected.uid ? 'Deleting…' : 'Delete Job'}
              </button>
            </div>
          </div>
