...
// --- Load existing selections & header on login (loader only) ---
async function loadExistingSelections(uid) {
  try {
    setLoading(true);

    // Load header (clientName)
    const headerRef = doc(db, 'selections', uid);
    const headerSnap = await getDoc(headerRef);
    if (headerSnap.exists()) {
      const hdr = headerSnap.data() || {};
      if (hdr.clientName) setClientName(hdr.clientName);
    }

    // Load categories/items -> local selections shape { [catName]: { Items: [...] } }
    const catsRef = collection(db, 'selections', uid, 'categories');
    const catsSnap = await getDocs(catsRef);
    const nextSelections = {};
    catsSnap.forEach(d => {
      const data = d.data() || {};
      const name = data.category || d.id;
      const items = Array.isArray(data.items) ? data.items : [];
      const clean = sanitizeItemsForFirestore(items);
      nextSelections[name] = { Items: clean };
    });

    // Apply to state
    setSelections(nextSelections);

    // If any category loaded, switch to the first one with items; otherwise keep current tab
    const loadedCats = Object.keys(nextSelections);
    if (loadedCats.length) {
      const withItems = loadedCats.find(c => Array.isArray(nextSelections[c].Items) && nextSelections[c].Items.length);
      const newTab = withItems || loadedCats[0];
      if (newTab && newTab !== tab) setTab(newTab);
    }
  } catch (e) {
    console.error('[loader] failed to load selections:', e);
  } finally {
    setLoading(false);
  }
}
...
useEffect(() => {
  ...
  // Loader-only: fetch header + categories/items for this user
  loadExistingSelections(user.uid);
